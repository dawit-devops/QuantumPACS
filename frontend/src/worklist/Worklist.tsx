...
import { Layout, Table, message, Button, Tag, Modal, Form, Select, Popconfirm, Tabs, Tooltip, Badge, Radio, Input, DatePicker } from 'antd';
import { EditOutlined, CloseCircleOutlined, CheckCircleOutlined, CalendarOutlined, TableOutlined } from '@ant-design/icons';
...
function Worklist() {
  ...
  let [stationFilter, setStationFilter] = useState<string | undefined>(undefined);
  ...
  const fetch = useCallback((params?: any) => {
    ...
    request('worklist', query).then((res: any) => {
      setLoading(false);
  ...
  useEffect(() => {
    fetch();
    request('worklist/station-aes', { method: 'GET' }).then((res: any) => {
  ...
  const stationOptionsFromEntries = useMemo(() => {
    const stations = new Set(data.map((e) => e.station_ae_title).filter(Boolean));
  ...
  const handleEdit = (entry: any) => {
    ...
    form.setFieldsValue({
      ...entry,
      station_ae_title: entry.station_ae_title ? [entry.station_ae_title] : undefined,
    ...
    form.validateFields().then((values: any) => {
      ...
      for (const key of ['patient_name', 'patient_birth_date', 'patient_sex', 'accession_number', 'requested_procedure_id', 'requested_procedure_desc', 'modality', 'station_ae_title']) {
        if (values[key] !== undefined && values[key] !== editingEntry[key]) {
          data[key] = Array.isArray(values[key]) ? values[key][0] : values[key];
        }
      }
  ...
  const handleMarkPerformed = (id: string) => {
    request(`worklist/${id}`, { data: { status: 'performed' } }).then(() => {
  ...
  const columns: any[] = [
    {
      title: 'Patient Name', dataIndex: 'patient_name', width: '14%',
      render: (v: string) => v || '-',
    },
    { title: 'Patient ID', dataIndex: 'patient_id', width: '8%' },
    { title: 'Accession #', dataIndex: 'accession_number', width: '9%' },
    { title: 'Proc ID', dataIndex: 'requested_procedure_id', width: '7%',
      render: (v: string) => v || '-',
    },
    { title: 'Modality', dataIndex: 'modality', width: '6%' },
    {
      title: 'Scheduled Date', dataIndex: 'scheduled_date', width: '9%',
      render: (d: string) => d || '-',
    },
    { title: 'Station AE', dataIndex: 'station_ae_title', width: '10%',
    ...
    {
      title: 'Study UID', dataIndex: 'study_uid', width: '10%',
      render: (v: string) => v ? <Tag style={{ margin: 0, fontFamily: 'monospace', fontSize: 11 }}>{v.slice(0, 20)}…</Tag> : '-',
    },
    {
      title: 'Performed', dataIndex: 'performed_at', width: '9%',
      render: (v: string) => v ? new Date(v).toLocaleString() : '-',
    },
    {
      title: 'Action', key: 'action', width: '11%',
      render: (_: any, record: any) => (
        <span style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          {record.status === 'scheduled' && (
            <>
              <Popconfirm title="Mark as performed?" onConfirm={() => handleMarkPerformed(record.id)}>
                <Tooltip title="Mark performed">
                  <CheckCircleOutlined style={{ cursor: 'pointer', color: '#16a34a', fontSize: 16 }} />
                </Tooltip>
              </Popconfirm>
              <Popconfirm title="Cancel this entry?" onConfirm={() => handleCancel(record.id)}>
                <Tooltip title="Cancel">
                  <CloseCircleOutlined style={{ cursor: 'pointer', color: '#dc2626', fontSize: 16 }} />
                </Tooltip>
              </Popconfirm>
            </>
          )}
          {record.status === 'performed' && (
            <Tooltip title={`Performed at ${record.performed_at ? new Date(record.performed_at).toLocaleString() : 'unknown'}`}>
              <CheckCircleOutlined style={{ color: '#16a34a', fontSize: 16, opacity: 0.5 }} />
            </Tooltip>
          )}
        </span>
      ),
  ...
  return (
    <Content style={{ padding: 24 }} role="main" id="main-content">
      <div aria-live="polite" aria-atomic="true" style={{ position: 'absolute', width: 1, height: 1, overflow: 'hidden', clip: 'rect(0,0,0,0)' }}>
        {loading ? 'Loading worklist' : `${filteredData.length} worklist entries`}
      </div>

      <div className="worklist-toolbar">
        <Button type="primary" onClick={() => { setEditingEntry(null); form.resetFields(); setVisible(true); }} aria-label="Create worklist entry">
          Create Entry
        </Button>
        ...
        <div className="worklist-toolbar-filters">
          <Input.Search
            ...
            onSearch={() => { setPagination((p: any) => ({ ...p, current: 1 })); fetch(); }}
            style={{ width: 200 }}
            size="small"
            allowClear
          />
          ...
          <Select
            ...
            style={{ minWidth: 160 }}
            size="small"
            options={[...new Set([...stationOptions, ...stationOptionsFromEntries])].map((s) => ({ value: s, label: s }))}
          />
        </div>
      </div>
      ...
      {selectedRowKeys.length > 0 && (
        <div className="worklist-batch-bar">
          ...
          <Button size="small" icon={<CheckCircleOutlined />} onClick={handleBatchPerformed} loading={batchLoading}>
            Mark Performed
          </Button>
          <Button size="small" icon={<CloseCircleOutlined />} onClick={handleBatchCancel} loading={batchLoading} danger>
            Cancel All
          </Button>
        </div>
      )}
      ...
      <PageState error={error} onRetry={() => fetch()}
        ...
        emptyMessage={statusTab !== 'all' ? `No ${statusTab} entries` : 'No worklist entries found'}
        emptyAction={
          <Button type="primary" onClick={() => { setEditingEntry(null); form.resetFields(); setVisible(true); }}>Create Entry</Button>
        }
      >
        {viewMode === 'table' ? (
          <Table
              ...
              <div key={date} className="calendar-day">
                ...
                {entries.map((entry) => (
                  <div
                    ...
                    role="button"
                    tabIndex={0}
                    onKeyDown={(e) => { if (e.key === 'Enter') handleEdit(entry); }}
                  >
                    <Tag color={STATUS_COLORS[entry.status]} style={{ margin: 0, flexShrink: 0 }}>
                      {entry.status}
                    </Tag>
                    <span style={{ fontWeight: 500, flex: 1 }}>{entry.patient_name || entry.patient_id}</span>
                    <span style={{ color: 'var(--text-secondary, #64748b)', fontSize: 13 }}>{entry.modality}</span>
                    {entry.scheduled_time && (
                      <span style={{ color: 'var(--text-secondary, #64748b)', fontSize: 13 }}>{entry.scheduled_time}</span>
                    )}
                    <span style={{ color: 'var(--text-secondary, #64748b)', fontSize: 13 }}>{entry.accession_number}</span>
                  </div>
                ))}
              </div>
            ))}
