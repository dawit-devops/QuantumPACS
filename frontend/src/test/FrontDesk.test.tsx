...
describe("Registration", () => {
  beforeEach(() => {
    ...
    mockCreateVisit.mockResolvedValue({ id: "v1", patient_id: "P001", status: "registered" });
  });
...
describe("Visits & Check-In", () => {
  ...
  it("lists visits and lets the user check one in", async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateVisit).toHaveBeenCalledWith("v1", { status: "checked_in" });
    });
  });
...
describe("WaitingQueue", () => {
  ...
  it("renders the privacy-projected queue and the HIPAA note", async () => {
    ...
    // Full names never appear on the board.
...
describe("AppointmentBooking", () => {
  ...
  it("surfaces a 409 slot conflict with a refresh", async () => {
