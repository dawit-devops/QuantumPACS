...
const mockWebhooks = [
  { id: 1, name: 'Slack', url: 'https://hooks.slack.com/abc', events: ['study.read'], active: true, last_triggered_at: '2026-07-01T12:00:00Z', last_status_code: 200 },
  { id: 2, name: 'PagerDuty', url: 'https://events.pagerduty.com/xyz', events: [], active: false, last_triggered_at: null, last_status_code: null },
];
const mockProviders = [
  { id: 'p1', issuer: 'https://accounts.google.com', client_id: 'g-client', scope: 'openid email', enabled: true, auto_provision: true },
  { id: 'p2', issuer: 'https://idp.example.com', client_id: 'e-client', scope: 'openid', enabled: false, auto_provision: false },
];
...
describe('Integrations', () => {
  beforeEach(() => {
    ...
    mockRequest.mockImplementation((url: string) => {
      if (url === 'webhooks') return Promise.resolve({ webhooks: mockWebhooks, available_events: mockEvents });
      if (url === 'oauth/providers') return Promise.resolve({ data: mockProviders });
  ...
  it('renders OAuth provider table on tab switch', async () => {
    ...
    expect(await screen.findByText('https://accounts.google.com')).toBeInTheDocument();
  ...
  it('shows webhook count', async () => {
    renderWithAuth(<Integrations />);
    await waitForTabs();
    expect(await screen.findByText('2 webhooks configured')).toBeInTheDocument();
  });
  ...
  it('creates a webhook via modal', async () => {
    ...
    mockRequest.mockImplementation((url: string, opts?: any) => {
      if (url === 'webhooks' && opts?.method === 'POST') return Promise.resolve({});
      if (url === 'webhooks') return Promise.resolve({ webhooks: mockWebhooks, available_events: mockEvents });
      if (url === 'oauth/providers') return Promise.resolve({ data: mockProviders });
      return Promise.resolve({});
    });
    ...
    await user.type(within(modal).getByLabelText('URL'), 'https://example.com/hook');
    await user.click(within(modal).getByText('OK'));
    await waitFor(() => {
      expect(mockRequest).toHaveBeenCalledWith('webhooks', expect.objectContaining({ method: 'POST' }));
    });
  });
  ...
  it('deletes a webhook', async () => {
    ...
    await waitFor(() => {
      expect(mockRequest).toHaveBeenCalledWith('webhooks/1', { method: 'DELETE' });
    });
  });
