...
const mockRequests = [
  { id: 1, created_at: '2026-07-01T12:00:00Z', method: 'GET', path: '/Patient/123', status_code: 200, duration_ms: 42, resource_type: 'Patient', caller: 'admin' },
  { id: 2, created_at: '2026-07-01T12:01:00Z', method: 'GET', path: '/ImagingStudy/456', status_code: 404, duration_ms: 15, resource_type: 'ImagingStudy', caller: 'system' },
];
...
describe('FhirMonitoring', () => {
  beforeEach(() => {
    ...
    mockRequest.mockImplementation((url: string) => {
      if (url.startsWith('fhir/admin/metrics')) return Promise.resolve(mockMetrics);
      if (url.startsWith('fhir/admin/requests')) return Promise.resolve({ requests: mockRequests, total: 2 });
  ...
  it('fetches metrics and requests on mount', async () => {
    ...
    expect(mockRequest).toHaveBeenCalledWith('fhir/admin/requests?limit=50&offset=0');
  });
  ...
  it('renders volume table', async () => {
    ...
    expect(screen.getAllByText('ImagingStudy').length).toBeGreaterThanOrEqual(1);
  });
