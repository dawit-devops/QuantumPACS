...
const mockConfig = { enabled: true, base_url: 'https://fhir.example.com', publisher: 'QuantumPACS', max_search_results: 100, log_retention_days: 90 };
const mockClients = [
  { id: 'c1', name: 'Epic', client_id: 'epic-client', active: true, grant_type: 'authorization_code', last_used: '2026-07-01T12:00:00Z' },
  { id: 'c2', name: 'Cerner', client_id: 'cerner-client', active: false, grant_type: 'authorization_code', last_used: null },
];
...
describe('FhirConfig', () => {
  beforeEach(() => {
    ...
    mockRequest.mockImplementation((url: string) => {
      ...
      if (url === 'fhir/admin/clients') return Promise.resolve({ clients: mockClients });
  ...
  it('creates a client via modal', async () => {
    ...
    mockRequest.mockImplementation((url: string, opts?: any) => {
      if (url === 'fhir/admin/clients' && opts?.method === 'POST') return Promise.resolve({ client_id: 'new-client', client_secret: 's3cret' });
      if (url === 'fhir/admin/clients') return Promise.resolve({ clients: mockClients });
      if (url === 'fhir/admin/config') return Promise.resolve(mockConfig);
      return Promise.resolve({});
    });
    ...
    await waitFor(() => {
      expect(mockRequest).toHaveBeenCalledWith('fhir/admin/clients', expect.objectContaining({ method: 'POST' }));
    });
  });
