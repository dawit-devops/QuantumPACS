...
const mockInfo = {
  qido: { enabled: true, response_format: 'application/fhir+json', pagination: 'cursor-based', features: { transfer_syntax: '*' }, modality_validation: true, valid_modalities_count: 50, endpoints: [{ method: 'GET', path: '/{study}', description: 'Query studies' }], search_params: [{ name: 'PatientID', type: 'string', description: 'Filter by patient' }] },
  wado: { enabled: true, response_format: 'application/dicom', endpoints: [{ method: 'GET', path: '/{study}/{series}/{instance}', description: 'Retrieve instance' }] },
  stow: { enabled: false, response_format: 'application/dicom', endpoints: [{ method: 'POST', path: '/{study}', description: 'Store instances' }] },
