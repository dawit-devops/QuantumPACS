...
const mockRules = [
  {
    id: '1', name: 'Route CT Chest', description: 'Route CT chest studies to fast storage',
    conditions: { modality: 'CT', study_description: { contains: 'CHEST' } },
    destination: 'replica_2', priority: 10, enabled: true,
    created_at: '2026-07-01T00:00:00Z', updated_at: '2026-07-01T00:00:00Z',
  },
  {
    id: '2', name: 'Route MR Brain', description: 'Route MR brain studies to warm storage',
    conditions: { modality: 'MR', study_description: { contains: 'BRAIN' } },
    destination: 'replica_3', priority: 5, enabled: false,
...
describe('RoutingRules', () => {
  beforeEach(() => {
    ...
    mockRequest.mockImplementation((url: string, opts?: any) => {
      ...
      return Promise.resolve({ data: mockRules, pagination: { page: 1, per_page: 50, total: 2, pages: 1 } });
    });
  ...
  it('calls request with correct endpoint on mount', async () => {
    renderWithAuth(<RoutingRules />);
    await waitForTable();
    expect(mockRequest).toHaveBeenCalledWith('routing', { page: 1, per_page: 50 });
  });
  ...
  it('delete rule calls API and refreshes', async () => {
    ...
    await waitFor(() => {
      expect(mockRequest).toHaveBeenCalledWith('routing/1', { data: undefined, method: 'DELETE' });
    });
  });
});
