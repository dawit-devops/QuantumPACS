...
const mockMessages = [
  { id: 'm1', created_at: '2026-07-01T12:00:00Z', message_type: 'ADT', event_type: 'A01', parse_status: 'ok', patient_id: 'P001', accession_number: 'ACC001', sending_facility: 'Hospital A' },
  { id: 'm2', created_at: '2026-07-01T12:05:00Z', message_type: 'ORM', event_type: 'O01', parse_status: 'failed', patient_id: 'P002', accession_number: 'ACC002', sending_facility: 'Hospital B' },
];
const mockMetrics = { total: 50, by_status: [{ parse_status: 'ok', count: 35 }, { parse_status: 'failed', count: 15 }], by_type: [{ message_type: 'ADT', event_type: 'A01', count: 30 }], by_facility: [{ sending_facility: 'Hospital A', count: 30 }] };
...
describe('Hl7Dashboard', () => {
  beforeEach(() => {
    ...
    mockRequest.mockImplementation((url: string) => {
      if (url.startsWith('hl7/admin/messages')) return Promise.resolve({ messages: mockMessages, total: 2 });
      if (url.startsWith('hl7/admin/metrics')) return Promise.resolve(mockMetrics);
  ...
  it('fetches messages on mount', async () => {
    renderWithAuth(<Hl7Dashboard />);
    await waitFor(() => {
      expect(mockRequest).toHaveBeenCalledWith('hl7/admin/messages?limit=50&offset=0');
    });
  });
