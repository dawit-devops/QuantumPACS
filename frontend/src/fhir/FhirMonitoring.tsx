...
import { Layout, Card, Row, Col, Statistic, Table, Tag, Button, DatePicker, Select, Space, Spin, Tabs, message } from 'antd';
...
function FhirMonitoring(props: any) {
  ...
  const handleExport = () => {
    ...
    if (resourceFilter) q += `&resource_type=${resourceFilter}`;
    if (statusFilter) q += `&status_min=${statusFilter}`;
    open(q).then(() => message.success('CSV export triggered')).catch(() => {});
  };

  const reqColumns = [
    { title: 'Time', dataIndex: 'created_at', key: 'created_at', render: (t: string) => new Date(t).toLocaleString() },
    { title: 'Method', dataIndex: 'method', key: 'method', render: (t: string) => <Tag>{t}</Tag> },
    ...
    { title: 'Duration', dataIndex: 'duration_ms', key: 'duration_ms', render: (t: number) => `${t}ms` },
    ...
    <Content className="fhir-monitoring" style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <Space>
          ...
          <Select value={period} onChange={v => { setPeriod(v); setOffset(0); }} style={{ width: 120 }}
            options={[
        ...
        <Space>
          <Button icon={<ReloadOutlined />} onClick={() => { fetchMetrics(); fetchRequests(); }}>Refresh</Button>
      ...
      <Row gutter={16} style={{ marginBottom: 16 }}>
        <Col span={6}>
          <Card><Statistic title="Total Requests" value={metrics?.total_requests || 0} /></Card>
        </Col>
        <Col span={6}>
          <Card><Statistic title="Error Rate" value={metrics?.error_rate || 0} suffix="%" precision={2}
            valueStyle={{ color: (metrics?.error_rate || 0) > 5 ? 'red' : 'green' }}
          /></Card>
        </Col>
        <Col span={6}>
          <Card><Statistic title="p50 Latency" value={metrics?.latency?.p50 || 0} suffix="ms" /></Card>
        </Col>
        <Col span={6}>
          <Card><Statistic title="p99 Latency" value={metrics?.latency?.p99 || 0} suffix="ms"
            valueStyle={{ color: (metrics?.latency?.p99 || 0) > 1000 ? 'red' : undefined }}
          /></Card>
        </Col>
      </Row>
      ...
      <Card title="Request Volume" size="small" style={{ marginBottom: 16 }}>
        {metrics?.volume?.length > 0 ? (
          <Table
            ...
            columns={[
              { title: 'Resource Type', dataIndex: 'resource_type', key: 'resource_type', render: (t: string) => t || 'Unknown' },
          ...
          <div style={{ color: 'var(--text-secondary)', padding: 24, textAlign: 'center' }}>
            No FHIR requests in this period. Advertise your FHIR endpoint to EHR systems to get started.
          </div>
        )}
      </Card>
      ...
      <Card title="Status Codes" size="small" style={{ marginBottom: 16 }}>
        {metrics?.status_codes?.length > 0 ? (
          <Table
            ...
            columns={[
              {
                title: 'Status Family', dataIndex: 'status_family', key: 'status_family',
                render: (t: number) => {
                  const color = t < 300 ? 'green' : t < 500 ? 'orange' : 'red';
          ...
          <div style={{ color: 'var(--text-secondary)', padding: 24, textAlign: 'center' }}>No data</div>
        )}
      </Card>

      {/* Recent Requests */}
      <Card title="Recent Requests" size="small"
        extra={
          <Space>
            <Select value={resourceFilter} onChange={setResourceFilter} allowClear placeholder="Resource Type" style={{ width: 150 }}
              options={[
                { value: '', label: 'All' },
            ...
            />
            <Select value={statusFilter} onChange={setStatusFilter} allowClear placeholder="Status" style={{ width: 120 }}
              options={[
                { value: '', label: 'All' },
            ...
            />
            <span style={{ color: 'var(--text-secondary)', fontSize: 12 }}>{requestsTotal} total</span>
          </Space>
        }
      >
        <Table
