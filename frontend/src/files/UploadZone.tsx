...
export function UploadZone({ reload }: UploadZoneProps) {
    ...
    uploadFile(
      ...
      .then(async (resp) => {
        try {
          ...
          if (json.duplicate) {
            updateFile(id, { status: 'done', progress: 100, error: 'File already exists' });
          } else if (json.error) {
            updateFile(id, { status: 'error', error: json.error, progress: 0 });
            return;
          }
        } catch {}
      ...
      .catch((err: Error) => {
        if (err.message === 'Cancelled') {
          ...
          updateFile(id, { status: 'error', error: err.message, progress: 0 });
        }
      });
  ...
  const handleRetry = useCallback((id: string) => {
    ...
    if (!originalFile) {
      updateFile(id, { status: 'error', error: 'Original file not available' });
      return;
    }
    const newController = new AbortController();
    updateFile(id, { status: 'pending', progress: 0, error: undefined, controller: newController });
    setTimeout(() => {
  ...
  const activeCount = queue.filter((f) => f.status === 'pending' || f.status === 'uploading').length;
    ...
    <div>
      <div
        ...
        style={{
          ...
          background: activeCount > 0 ? 'var(--bg-active, #f0fdfa)' : 'transparent',
        }}
      ...
      >
        <input
          ...
          style={{ display: 'none' }}
        />
        <InboxOutlined style={{ fontSize: 48, color: 'var(--color-primary, #0891B2)', marginBottom: 8 }} />
        <p style={{ margin: 0, fontWeight: 500 }}>Drag & drop DICOM files here</p>
