...
function RejectAnalysisPanel() {
  ...
  const modCols = [
    ...
    { title: "Reject Rate", dataIndex: "reject_rate", key: "rate",
      render: (v: number) => (
        <span style={{ color: v > 10 ? "#ff4d4f" : v > 5 ? "#fa8c16" : "#52c41a" }}>
          {v}%
        </span>
      ) },
  ];
    ...
    <PageState error={error} onRetry={fetch} loading={loading}>
      {data && (
        <>
          ...
          <Table rowKey="tech" columns={techCols}
            dataSource={data.by_technologist} pagination={false}
            size="small" style={{ marginBottom: 16 }} />
...
function DoseTrackingPanel() {
  ...
  const modCols = [
    ...
    { title: "Avg DLP", dataIndex: "avg_dlp", key: "avg_dlp",
      render: (v: number) => `${v} mGy·cm` },
    { title: "Max DLP", dataIndex: "max_dlp", key: "max_dlp",
      render: (v: number) => `${v} mGy·cm` },
    { title: "ACR DLP Limit", dataIndex: "acr_benchmark_dlp", key: "bench",
      render: (v: number | null) => v != null ? `${v} mGy·cm` : "—" },
    { title: "Exceedances", dataIndex: "dlp_exceedances", key: "exc",
      render: (v: number) => (
            ...
            <>
              ...
              <Table
                ...
                columns={[
                  ...
                  { title: "Modality", dataIndex: "modality", key: "mod",
                    render: (v: string) => <Tag>{v}</Tag> },
                  { title: "DLP", dataIndex: "dose_dlp", key: "dlp",
                    render: (v: number) => `${v} mGy·cm` },
                  { title: "ACR Limit", dataIndex: "acr_benchmark_dlp", key: "bench",
                    render: (v: number) => `${v} mGy·cm` },
...
function TechMetricsPanel() {
  ...
  const cols = [
    ...
    { title: "Avg DLP", dataIndex: "avg_dlp", key: "dlp",
      render: (v: number) => v > 0 ? `${v}` : "—" },
    { title: "Protocol Adherence", dataIndex: "protocol_adherence_pct", key: "adh",
      render: (v: number) => (
...
function ProtocolCompliancePanel() {
  ...
  const cols = [
    ...
    { title: "Modality", dataIndex: "modality", key: "mod",
      render: (v: string) => <Tag>{v}</Tag> },
    { title: "Body Part", dataIndex: "body_part", key: "bp",
      render: (v: string) => v || "—" },
    { title: "Reviews", dataIndex: "total_reviews", key: "total" },
    { title: "Compliance %", dataIndex: "compliance_pct", key: "pct",
      render: (v: number) => (
        <span style={{ color: v < 80 ? "#ff4d4f" : v < 90 ? "#fa8c16" : "#52c41a" }}>
          {v != null ? `${v}%` : "—"}
        </span>
...
function TrendsPanel() {
  ...
  const fetch = useCallback(() => {
    ...
    getQATrends(granularity)
      .then((d) => { setLoading(false); setData(d.data); })
      .catch((e: any) => { setLoading(false); setError(e.message); });
  }, [granularity]);
