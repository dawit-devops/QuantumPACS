...
function Visits() {
  ...
  const loadDetail = useCallback((visit: Visit) => {
    ...
    Promise.all([
      ...
      .catch((e: any) => message.error(e.message || "Failed to load visit"))
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (s: string) => (
          <Tag
            color={s === "synced" ? "green" : s === "failed" ? "red" : "default"}
          >
            {s || "pending"}
          </Tag>
        ),
  ...
  const counts = useMemo(() => {
    ...
    for (const v of data) c[v.status] = (c[v.status] || 0) + 1;
    return c;
  }, [data]);
        ...
        {detailLoading ? (
          ...
          <>
            ...
            <div className="fd-visit-section">
              ...
              {orders.length === 0 ? (
                <Alert type="info" showIcon message="No orders yet — add the referring order below." />
              ) : (
                <Timeline
                  items={orders.map((o) => ({
                    dot: <ClockCircleOutlined />,
                    color: o.urgency === "stat" ? "red" : o.urgency === "urgent" ? "orange" : "blue",
                    children: (
                      <>
                        <b>{o.requested_procedure}</b>
                        {o.urgency && (
                          <Tag color={o.urgency === "stat" ? "red" : o.urgency === "urgent" ? "orange" : "default"} style={{ marginLeft: 8 }}>
                            {o.urgency.toUpperCase()}
                          </Tag>
                        )}
                        <div style={{ color: "var(--text-secondary)", fontSize: 12 }}>
                          {o.indication || "No indication"} · Dr. {o.referring_physician || "—"}
                        </div>
                      </>
                    ),
                  }))}
              ...
              {canWrite && (
                <Form form={orderForm} layout="vertical" style={{ marginTop: 12 }}>
                  <Form.Item name="requested_procedure" label="Requested procedure" rules={[{ required: true, message: "Procedure required" }]}>
                    <Input placeholder="e.g. CT CHEST W CONTRAST" />
                  </Form.Item>
                  <div style={{ display: "flex", gap: 12 }}>
                    <Form.Item name="urgency" label="Urgency" style={{ flex: 1 }} initialValue="routine">
                      <Select
                        options={["routine", "urgent", "stat"].map((u) => ({ value: u, label: u }))}
                      />
                    </Form.Item>
                    <Form.Item name="referring_physician" label="Referring physician" style={{ flex: 1 }}>
                      <Input placeholder="Last^First" />
                    </Form.Item>
                  </div>
                  <Form.Item name="indication" label="Indication">
                    <Input.TextArea rows={2} placeholder="Clinical indication" />
                  </Form.Item>
                  <Button type="primary" onClick={submitOrder} loading={saving} icon={<MedicineBoxOutlined />}>
                    Add Order
                  </Button>
                </Form>
              )}
            ...
            <div className="fd-visit-section">
              ...
              {consents.length === 0 ? (
                <Alert type="warning" showIcon message="No required consents seeded for this visit." />
              ) : (
                consents.map((c) => (
                  <div key={c.id} className="fd-patient-result" style={{ marginBottom: 8 }}>
                    <div>
                      <b>{c.consent_type.replace(/_/g, " ")}</b>
                      <div className="fd-patient-meta">
                        {c.file_name || "No file"} · {c.attached_at ? new Date(c.attached_at).toLocaleString() : "not yet attached"}
                      </div>
                    </div>
                    <Tag color={c.status === "attached" ? "green" : c.status === "missing" ? "red" : "orange"}>
                      {c.status}
                    </Tag>
                  </div>
                ))
              ...
              {canWrite && (
                <Form form={consentForm} layout="vertical" style={{ marginTop: 12 }}>
                  <div style={{ display: "flex", gap: 12 }}>
                    <Form.Item name="consent_type" label="Consent type" style={{ flex: 1 }} initialValue="general_consent">
                      <Select options={CONSENT_TYPES} />
                    </Form.Item>
                    <Form.Item name="file_name" label="File name" style={{ flex: 1 }} rules={[{ required: true, message: "File name required" }]}>
                      <Input placeholder="e.g. consent-signed.pdf" />
                    </Form.Item>
                  </div>
                  <Button type="primary" onClick={submitConsent} loading={saving} icon={<SafetyCertificateOutlined />}>
                    Attach Consent
                  </Button>
                </Form>
              )}
            ...
            <div className="fd-visit-section">
              ...
              {insurance.length === 0 ? (
                <Alert type="info" showIcon message="No insurance record yet." />
              ) : (
                insurance.map((i) => (
                  <div key={i.id} className="fd-patient-result" style={{ marginBottom: 8 }}>
                    <div>
                      <b>{i.policy_number || "No policy #"}</b>
                      <div className="fd-patient-meta">
                        Guarantor: {i.guarantor_name || "—"} · {i.authorization_status || "none"}
                      </div>
                    </div>
              ...
              {canWrite && (
                <Form form={insuranceForm} layout="vertical" style={{ marginTop: 12 }}>
                  <div style={{ display: "flex", gap: 12 }}>
                    <Form.Item name="policy_number" label="Policy #" style={{ flex: 1 }}>
                      <Input placeholder="Policy number" />
                    </Form.Item>
                    <Form.Item name="guarantor_name" label="Guarantor" style={{ flex: 1 }}>
                  ...
                  <Button type="primary" onClick={submitInsurance} loading={saving} icon={<InsuranceOutlined />}>
                    Save Insurance
                  </Button>
                </Form>
              )}
