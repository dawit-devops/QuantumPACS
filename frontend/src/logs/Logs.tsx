...
import { Layout, Table, message, Tag, Button, Switch, DatePicker, Space, Typography, Input, Select, Badge } from 'antd';
...
const EVENT_GROUPS: Record<string, string[]> = {
  'Data Access': ['study.read', 'study.download', 'series.read', 'instance.read', 'instance.download'],
  'Data Modification': ['study.updated', 'study.anonymized', 'study.deleted', 'series.updated', 'instance.annotations_changed', 'instance.tags_edited'],
  'Auth & Session': ['auth.login', 'auth.login_failed', 'auth.logout', 'auth.token_refreshed', 'auth.password_changed'],
  'User Management': ['user.created', 'user.updated', 'user.deactivated', 'user.reactivated', 'user.deleted'],
  'Tenant Management': ['tenant.created', 'tenant.updated', 'tenant.quarantined', 'tenant.decommissioned', 'tenant.storage_quota_changed'],
  'Replica Management': ['replica.created', 'replica.updated', 'replica.deleted', 'replica.master_changed', 'replica.sync_status_changed'],
  'System': ['system.config_changed', 'system.backup_completed', 'system.backup_failed', 'system.maintenance_mode'],
};
...
function Logs() {
  ...
  let [cursorMap, setCursorMap] = useState<Record<number, number | null>>({ 1: null });
  ...
  const buildQuery = useCallback((extra: Record<string, any> = {}) => {
    ...
    if (dateRange) { q.date_from = dateRange[0]; q.date_to = dateRange[1]; }
  ...
  const pollNew = async () => {
    ...
    try {
      ...
      const newItems = (res.data || []).filter((item: any) => item.id > (latestIdRef.current || 0));
      if (newItems.length === 0) return;
      const ids = newItems.map((i: any) => i.id);
      const isAtTop = tableRef.current ? tableRef.current.scrollTop < 100 : true;
      if (isAtTop) {
        setData((prev) => [...newItems, ...prev]);
        setNewEventIds((prev) => {
          const next = new Set(prev);
          ids.forEach((id: number) => next.add(id));
          setTimeout(() => setNewEventIds((p) => { const n = new Set(p); ids.forEach((id: number) => n.delete(id)); return n; }), 2000);
          return next;
        });
  ...
  const handlePageChange = (newPage: number) => {
    setCursorMap((prev) => {
      ...
      return { ...prev, [newPage]: data.length > 0 ? data[data.length - 1].id : null };
    });
    setPage(newPage);
  };
  ...
  const handleActorSearch = (value: string) => {
    ...
    actorDebounceRef.current = setTimeout(async () => {
      try {
        const res = await request('logs/actors', { query: { search: value, limit: 10 } });
        setActors(res.data || []);
      } catch { }
    }, 300);
  };
  ...
  const exportCsv = () => {
    const header = 'Timestamp,Actor,Event Type,Resource Type,Resource ID,Description,Tenant,Payload\n';
    const rows = data.map((row: any) => {
      ...
      return [
        row.created_at, row.actor, row.event_type, row.resource_type,
        row.resource_id, row.description, row.tenant, escape(row.payload),
      ].join(',');
    }).join('\n');
  ...
  const columns: any[] = [
    {
      title: 'Timestamp', dataIndex: 'created_at', width: '16%',
      render: (d: string) => d ? new Date(d).toLocaleString() : '-',
    },
    {
      title: 'Actor', dataIndex: 'actor', width: '12%',
      render: (a: string) => a === 'system' ? <Tag color="default">system</Tag> : a,
    },
    {
      title: 'Event Type', dataIndex: 'event_type', width: '16%',
      render: (t: string) => {
        if (!t) return '-';
        const color = EVENT_TYPE_COLORS[t] || 'default';
        const group = Object.entries(EVENT_GROUPS).find(([, types]) => types.includes(t))?.[0];
        return <Tag color={color} title={group}>{t}</Tag>;
      },
    },
    {
      title: 'Resource Type', dataIndex: 'resource_type', width: '12%',
      render: (r: string) => r || '-',
    },
    {
      title: 'Resource ID', dataIndex: 'resource_id', width: '16%',
      render: (id: string) => id ? <Text copyable style={{ fontSize: 12 }}>{id}</Text> : '-',
    },
    {
      title: 'Description', dataIndex: 'description', width: '28%',
      render: (d: string) => d || '-',
    },
  ];
    ...
    <Content style={{ padding: 24, display: 'flex', flexDirection: 'column', height: '100%' }}>
      <div style={{ marginBottom: 12, flexShrink: 0 }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, alignItems: 'center', marginBottom: 8 }}>
          <Space wrap size={[4, 4]}>
            {Object.entries(EVENT_GROUPS).map(([group, types]) =>
              types.map((type) => (
                <Tag
                  key={type}
                  color={activeFilters.includes(type) ? (EVENT_TYPE_COLORS[type] || 'blue') : 'default'}
          ...
          <Button size="small" type="link" onClick={() => setEventTypeFilter([])} style={{ fontSize: 11 }}>
            Clear
          </Button>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          <Input
            ...
            style={{ width: 180 }}
            allowClear
            onClear={() => { setActorFilter(''); setActors([]); }}
          />
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
            {streaming && (
              <Badge status="processing" text={<Text style={{ fontSize: 12, color: '#52c41a' }}>Live</Text>} />
            )}
            <span style={{ fontSize: 12 }}>Live</span>
            <Switch size="small" checked={streaming} onChange={setStreaming} />
            <Button size="small" icon={<DownloadOutlined />} onClick={exportCsv} disabled={data.length === 0}>
              CSV
            </Button>
            <Button size="small" icon={<ReloadOutlined />} onClick={() => fetch({ cursor: cursorMap[page] || null })}>
              Refresh
            </Button>
          </div>
        </div>
        ...
        >
          <Table
            ...
            expandedRowRender={(record: any) => (
              <pre style={{ margin: 0, fontSize: 11, whiteSpace: 'pre-wrap', maxHeight: 300, overflow: 'auto' }}>
