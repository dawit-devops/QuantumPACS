...
function Portal() {
  ...
  // R1-05: sequence-guard patient loads — switching tabs quickly must never
  // let a slower earlier response render another patient's PHI.
  ...
  const loadPatient = useCallback((patientId: string) => {
    ...
    setBundle(null);
    setOrders([]);
    Promise.all([getPortalPatient(patientId), getPortalOrders(patientId)])
      .then(([b, o]) => {
  ...
  const orderColumns = [
    ...
    },
    {
      ...
      render: (v: string) => (
        <Tag
          color={v === "stat" ? "red" : v === "urgent" ? "orange" : "default"}
        >
          {v || "routine"}
        </Tag>
      ),
  ...
  return (
    <Content style={{ padding: 24 }} role="main">
      ...
      {loadingScope ? (
        ...
        // R1-15: a failed scope load must not masquerade as an empty share —
        // surface the error with a retry instead of the empty-state message.
        <Card>
          <Alert
            type="error"
            ...
            <Alert
              ...
              action={
                <Button
                  size="small"
                  onClick={() =>
                    activePatientId && loadPatient(activePatientId)
                  }
                >
                  Retry
                </Button>
