...
export function RuleConditionBuilder({
  ...
  useEffect(() => {
    if (value && Object.keys(value).length > 0) {
      ...
      setRows(
        parsed.length > 0
          ? parsed
          : [{ field: "modality", op: "equals", value: "" }],
      );
    } else {
  ...
  const removeRow = (index: number) => {
    ...
    setRows(
      next.length > 0 ? next : [{ field: "modality", op: "equals", value: "" }],
    );
    emit(
      next.length > 0 ? next : [{ field: "modality", op: "equals", value: "" }],
    );
      ...
      {rows.map((row, i) => (
        ...
        >
          <Select
            value={row.field}
            onChange={(v) =>
              updateRow(i, { field: v, op: "equals", value: "" })
            }
            style={{ minWidth: 160 }}
          ...
          <Select
            ...
            style={{ minWidth: 110 }}
            options={CONDITION_OPS.map((o) => ({
              value: o,
              label: OP_LABELS[o],
            }))}
          />
          {row.op !== "exists" &&
            valueInput(row.field, row.op, row.value, (v) =>
              updateRow(i, { value: v }),
            )}
          <Button
            type="text"
