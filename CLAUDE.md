...
- **Weasis manifest 404 / "failed to download manifest"**: the weasis-pacs-connector activates an archive only if `arc.config.list` is present in `weasis-pacs-connector.properties` (the copy in WildFly `standalone/configuration/` shadows the war's bundled default). Missing key → zero archives activated → `ManifestBuilder` logs "Empty patient list" → `RequestManifest` 404s with `IndexOutOfBoundsException`. Keep `arc.config.list=dicom-dcm4chee-arc.properties` in `docker/dcm4chee/weasis-pacs-connector.properties` (fix commit `87ac647`). Verify a manifest builds: `curl` the `weasis-pacs-connector/weasis?studyUID=<uid>&cdb` 302 Location, then fetch the embedded `RequestManifest` URL (expect 200 + JNLP XML with `<Patient>`/`<Instance>`).
