...
- [ ] Confirm dcm4chee-arc MWL SCP device is enabled in LDAP (`dcm4chee-arc` device, MWL SCP role). dcm4chee-arc serves Worklist C-FIND via DIMSE only — there is **no HTTP `/mwl` endpoint** (verified 404).
- [ ] Assert sync latency: entry appears in archive within the sync worker interval (configurable; default check at 5 s).
...
|---|---|---|
| Archive MWL SCP not enabled in LDAP | R5 parity test fails at first step | LDAP device config check first; enable via ldapmodify if absent |
...
| Sync latency flakiness in parity test | Flaky CI | Poll with deadline, not fixed sleep |
| `hl7_mllp_port` 12580 vs documented default 12579 | Doc drift | Keep config.local.yaml authoritative; note in doc |
| Phase 3 PR #119 not merged yet | Branch base divergence | Create branch after merge; rebase if needed |
