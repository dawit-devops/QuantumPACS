...
|---|---|---|---|---|---|---|---|---|---|
| 1 | Registration | /frontdesk/registration | REGISTRATION_READ | Patient search/create/register | GET /api/patients/search?q → 200; POST /api/patients → 200 (PATIENT_WRITE); GET /api/ris/patients/search?q → 200 | PENDING | | | |
| 2 | Today's Schedule | /frontdesk/schedule | SCHEDULE_READ | Day appointments | GET /api/ris/appointments?date → 200 | PENDING | | | |
| 3 | Waiting Queue | /frontdesk/queue | QUEUE_READ | Privacy queue | GET /api/frontdesk/queue → 200 | PENDING | | | |
| 4 | Patient Search | (action) | PATIENT_READ | Global search overlay | GET /api/patients/search?q → 200 | PENDING | | | |
| 5 | Orders | /orders | ORDER_READ | Order list | GET /api/ris/orders?offset&limit&q → 200 | PENDING | | | |
| 6 | Care Plans | /care-plans | PATIENT_READ | Care plan board | GET /api/ris/care-plans?offset&limit&q → 200 | PENDING | | | |
| 7 | Communications | /communications | PATIENT_READ | Patient comm log | GET /api/ris/communications?patient_id&limit → 200 | PENDING | | | |
