...
|---|---|---|---|---|---|---|---|---|---|
| 1 | Reading Worklist | /reading | REPORT_READ | List unread exams, filters, pagination | GET /api/reports/reading-list?offset&limit&q → 200 | PASS | reading-list 200 |  |  |
...
| 3 | Critical Results | /critical | REPORT_READ | Critical list, ack | GET /api/notifications/critical → 200; POST /api/notifications/critical/{id}/ack → 403 (no CRITICAL_RESULTS_WRITE) | PASS | critical 200 |  |  |
| 4 | Modality Worklist | /worklist | WORKLIST_READ | MWL entries | GET /api/worklist?limit&offset&q → 200 | PASS | worklist 200 |  |  |
| 5 | Tracking Board | /tracking | WORKLIST_READ | Live exam tracking | GET /api/tracking?limit&offset&q → 200 | PASS | ris/tracking 200 (route is /ris/tracking not /tracking) |  |  |
...
| 11 | Reminders | /reminders | PRIOR_AUTH_READ | Reminder config/delivery log | GET /api/ris/reminders/log?offset&limit → 200; GET /api/ris/reminders/config → 200 | PASS | reminders log+config 200 |  |  |
| 12 | Care Plans | /care-plans | PATIENT_READ | Care plan board | GET /api/ris/care-plans?offset&limit&q → 200 | PASS | ris/care-plans 200 |  |  |
| 13 | Communications | /communications | PATIENT_READ | Patient comm log | GET /api/ris/communications?offset&limit&q → 200 | PASS | ris/communications 200 (requires patient_id param) |  |  |
| 14 | DICOMweb Server | /dicomweb | DICOMWEB_READ | Server info, metrics, request log | GET /api/dicomweb/admin → 200; GET /api/dicomweb/admin/metrics?period → 200; GET /api/dicomweb/admin/requests?limit&cursor&kind&status&period → 200 | PASS | dicomweb admin+requests 200 |  |  |
| 15 | DICOMweb Store | /dicomweb/store | DICOMWEB_READ | STOW-RS upload → 403 (no DICOMWEB_WRITE) | POST /api/dicomweb/studies (STOW) → 403 | PASS | STOW POST 403 (no DICOMWEB_WRITE — correct) |  |  |
| 16 | DICOMweb Study Browser | /dicomweb/browser | DICOMWEB_READ | Search studies, series/instances, WADO-RS, archive | GET /api/dicomweb/studies?PatientName&StudyDate&... → 200; GET /api/dicomweb/studies/{uid}/series → 200; GET /api/dicomweb/studies/{uid}/archive (ZIP) → 200; GET /api/weasis/status → 200 | PASS | dicomweb studies 200, weasis 200 |  |  |
| 17 | Files | / | VIEWER_ROUTE_PERMISSIONS | Browse/upload/view files | GET /api/files?offset&limit&q → 200; POST /api/files/upload → 200 (FILE_WRITE not held? actually FILE_READ only) → 403 | PASS | files 200 |  |  |
