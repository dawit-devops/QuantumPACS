# Architecture Decision Records

| # | Title | Status |
|---|-------|--------|
| 001 | [Strangler Fig Modernization Strategy](ADR-001-strangler-fig-modernization.md) | Accepted |
| 002 | [Starlette Backend Framework](ADR-002-starlette-backend-framework.md) | Accepted |
| 003 | [JWT Authentication](ADR-003-jwt-authentication.md) | Accepted |
| 004 | [PostgreSQL Database](ADR-004-postgresql-database.md) | Accepted |
| 005 | [REST API Design with Pydantic Validation](ADR-005-api-design-rest-pydantic.md) | Accepted |
| 006 | [Frontend Architecture — React, Vite, Ant Design, Cornerstone3D](ADR-006-frontend-react-vite-antd.md) | Accepted |
| 007 | [Storage Backend Multi-Tier](ADR-007-storage-backend-multi-tier.md) | Accepted |
| 008 | [Security Architecture](ADR-008-security-architecture.md) | Accepted |
| 009 | [Repository Organization and Branching Strategy](ADR-009-repository-organization-and-branching-strategy.md) | Accepted |
| 010 | [QuantumPACS Brand Identity](ADR-010-quantumpacs-brand-identity.md) | Accepted |
| 011 | [DICOM Modality Worklist (MWL) SCP](ADR-011-dicom-mwl-scp.md) | Accepted |
| 012 | [HL7 v2.x ADT/ORM Ingestion](ADR-012-hl7-adt-orm-ingestion.md) | Accepted |
| 013 | [FHIR R4 API for EHR Integration](ADR-013-fhir-r4-api.md) | Accepted |
| 014 | [Modular Monolith with Deferred Microservices](ADR-014-modular-monolith-deferred-microservices.md) | Accepted |
| 015 | [Redis Streams as the v3 Message Bus](ADR-015-redis-streams-message-bus.md) | Accepted |
| 016 | [Database-Per-Tenant Multi-Tenancy](ADR-016-database-per-tenant-multi-tenancy.md) | Accepted |
| 017 | [OAuth/OIDC + RBAC for v3 Authentication and Authorization](ADR-017-oauth-oidc-rbac-auth.md) | Accepted |
| 018 | [DICOMweb API at /api/v2/dicomweb/](ADR-018-dicomweb-api.md) | Accepted |
| 019 | [FHIR R4 + HL7 v2.x Integration in v3](ADR-019-fhir-hl7-integration.md) | Accepted |
| 020 | [Observability Stack — Structured Logs + OpenTelemetry + Prometheus](ADR-020-observability-stack.md) | Accepted |
| 021 | [Testing Strategy — Integration-First + IHE Conformance + Load Gates](ADR-021-testing-strategy.md) | Accepted |
| 022 | [Git Branching Strategy — Phased Git Flow for v3](ADR-022-git-branching-strategy.md) | Accepted |
| 023 | [WebSocket Real-Time Events](ADR-023-websocket-real-time-events.md) | Accepted |
| 024 | [Share Links — Temporary-Key Auth](ADR-024-share-links-tempkey-auth.md) | Accepted |
| 025 | [Token Storage — HttpOnly Cookies + Headers](ADR-025-token-storage-http-cookie.md) | Accepted |
| 026 | [Tenant Data-Plane Wiring — Completing the DB-Per-Tenant Architecture](ADR-026-tenant-data-plane-wiring.md) | Accepted |
| 027 | [Exam→Imaging Bridge and the Split Reading Console](ADR-027-exam-imaging-bridge-reading-console.md) | Accepted |
