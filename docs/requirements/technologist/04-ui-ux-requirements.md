# UI/UX Requirements — Radiology Technologist (R06)
...
|--------|----|-------------|------------|
| Worklist | S-R06-01 | Sidebar → "Worklist" | Primary view; auto-refreshing table |
| Exam Detail | S-R06-02 | Click exam row in worklist | Slide-out panel or full-screen |
| Acquisition View | S-R06-03 | Exam detail → "Start Protocol" | Full-screen image preview + QA overlay |
| Dose Monitor | S-R06-04 | Acquisition view sidebar | Persistent dose panel |
| Safety Check | S-R06-05 | Before contrast administration | Modal overlay |
| Incident Log | S-R06-06 | From QA panel or worklist | Modal overlay |
| Protocol Override | S-R06-07 | From protocol panel | Modal overlay |

### Navigation Hierarchy
```
Sidebar
├── Worklist (S-R06-01) ──── Primary view
├── Exam Detail (S-R06-02) ──── From worklist click
│   ├── Acquisition View (S-R06-03) ──── From exam detail
│   │   ├── Dose Monitor (S-R06-04) ──── Sidebar
│   │   ├── Safety Check (S-R06-05) ──── Modal before contrast
│   │   └── Incident Log (S-R06-06) ──── From QA panel
│   └── Protocol Override (S-R06-07) ──── From protocol panel
...
### ProtocolOverride (S-R06-07)
| Component | Default | Loading | Empty | Error | Success | Disabled |
|-----------|---------|---------|-------|-------|---------|----------|
| ProtocolOverride | Modal (opens from protocol panel) | — | "No override needed" message | Red inline error | Override confirmed with justification logged | "Confirm Override" disabled until justification entered |
...
|----------------|----------------------|-------------|
| `acquisition-stat-bg` | `rgba(239, 68, 68, 0.1)` | Background for STAT exam blocks |
...
- `Modal` (existing) — used for safety check, incident log, protocol override
...
- ARIA labels: `aria-label="Worklist for {technologist_name}"`, `aria-label="Exam {accession} - {modality} - {priority}"`
...
- **Progressive disclosure**: Exam details shown on click, not inline; advanced options (override, incident log) in expandable panels
