# End-to-End Workflow Maps — Radiology Technician (R07)

## Workflow W1: Exam Preparation and Patient Positioning (frequency: per exam, criticality: high)

```mermaid
sequenceDiagram
    actor User as R07 Technician
    participant UI as Worklist + Exam Detail
    participant API as Backend API
    participant DB as PostgreSQL
    participant PACS as PACS Archive
    User->>UI: Open worklist (GET /api/v2/worklists/technician)
    UI->>API: GET /api/v2/worklists/technician
    API->>DB: SELECT exams WHERE assigned_to=R07 AND status='pending'
    ...
    User->>UI: Position patient per protocol instructions (DR/CR: standing/table; Mammo: seated)
...
- Patient identity verification requires technician to manually confirm, adding a step before acquisition
- Protocol parameters may conflict with patient safety (contrast allergy) — requires careful review
- Positioning instructions vary by modality (DR/CR vs. Mammography) — must be clear and modality-specific
- Incorrect positioning leads to repeat exams — cognitive load for positioning decisions

### Error & Exception Paths
- **Patient mismatch**: If patient initials don't match expected, technician can "Swap Patient" and re-search
...
### Friction & Cognitive Load Points
- Real-time QA requires technician to evaluate image quality while continuing acquisition — split attention
- Reject reason selection requires knowing standardized codes; incorrect codes affect QA metrics
- Cumulative dose tracking requires technician to monitor running total during exam
- Fluoroscopy live imaging requires continuous monitoring of dose and image quality
...
sequenceDiagram
    ...
    loop During acquisition
        UI->>UI: Update dose panel with each acquisition (DAP, DLP, kVp, mAs, SID)
...
### Friction & Cognitive Load Points
- Technician must monitor cumulative dose in real-time while acquiring images — cognitive load
...
- Fluoroscopy DAP tracking is continuous and can accumulate quickly — requires constant monitoring
...
sequenceDiagram
    ...
    participant PACS as PACS Archive
    User->>UI: Click "Complete Exam"
    UI->>API: POST /api/v2/exams/{id}/complete {dose_data, view_compliance, reject_count}
    API->>DB: UPDATE exam status='complete'; INSERT dose log; UPDATE view compliance
    DB-->>API: OK
...
### Friction & Cognitive Load Points
- Technician must ensure all required fields are complete before marking exam done (dose, views, rejects)
...
- Mammography exams require ACR-compliant dose reporting — additional data fields

### Error & Exception Paths
- **Completion blocked by missing data**: If dose or view compliance is incomplete, show "Please complete all required fields" and highlight missing fields
- **PACS push fails**: Images queued for retry; show "Images pending archive" banner; auto-retry every 30s
...
### Friction & Cognitive Load Points
- Technician must select correct reject reason code from standardized list
...
- Fluoroscopy incidents (contrast extravasation) require immediate notification

### Error & Exception Paths
- **Reject reason not selected**: Show "Please select a reject reason" validation error
- **Notification to R05/R12 fails**: Log the failure; retry notification; technician can manually notify
