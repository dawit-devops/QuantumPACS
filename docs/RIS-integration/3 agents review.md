...
i want you to map-out all remaining implementation from the 3 agent review, then draft a TDD implementation plan that follows their dependency graph.
I want you to proceed implimntation of the next sprint that follows The TDD imentation pipeline for full featured RIS integration in to existing code base following the dependency graph and consolidated sprint planing.
task 1: draft a TDD implementation plan that follows their dependency graph.(prioritized TDD pipeline aligned implementations plan (scoped to ris-integration-spec-sprint S6-S7))
task 2: invoke the TDD skill and impliment.

next i want you to conduct an in-depth rereview and audit of the sprint implementations of (S11 annd S12)  against the consolidated sprint plan full featured RIS integration in to existing code base
